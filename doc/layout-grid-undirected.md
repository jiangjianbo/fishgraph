...
`expand(i, gw, gh)`：释放 i 自身的质点覆盖格 → **向右插 gw−1 列、向下插
gh−1 行**（`shiftLine`：at 处开辟新行/列，该侧全部占用格与锚点整体平移
一格让位）→ 标记 i 的完整覆盖区。锚点不动、只向右/下扩张，与质点布局的
"邻居贴邻"语义兼容：扩张推走的正是 i 的右/下方邻居。
...
扩张推走邻居且不重叠）；走线正交 + 首尾为端点中心 + 中段不穿第三方 AABB；
确定性（同图同参逐位一致，含 waypoints）；rebuild 重放。
