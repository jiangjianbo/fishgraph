...
记节点 i、j 的**表面间隙** `g`（统一形状口径，2026-09-23 起）：圆-圆对 =
中心距减两个包围半径 `|pᵢ − pⱼ| − rᵢ − rⱼ`；其余组合（矩形容器、rect/ellipse
节点及混合对）= **外接矩形（AABB）表面间隙**（分离时为两矩形最短表面距离，
重叠时为负的浅轴穿透深度，由接触弹簧推离）。
...
- **subgraph**（`SubgraphSpec`）：物化为 `LayoutSubgraphNode` 容器节点（Compound
  Node），是真正的物理实体，与普通节点一起参与全局力场（质量归一，外部
  连线连到容器 id）。容器的**真实形状 = 成员实占包围盒 + `padding` 的动态
  矩形**（世界分块种入/终局对齐、分组层级量测时刷新）——渲染矩形、力学
  外接矩形与连线贴合共用这一份尺寸（统一形状计算，2026-09-23 起）；声明
  的 `shape` 只作成员钳制上界（`declaredShape`）与成员初始分布画布，不参与
  力学占位。成员被钳制（运动学约束）在声明形状边界内（容器与成员之间无
  斥力——成员在容器内不算穿透）。
  **渲染约定**：容器在 `layout.subgraphViews` 中（不再是 `nodeViews` 的一部分），
  绘制时应**作为背景层最先画**（成员与其它节点画在其上），否则容器矩形会盖住内部节点；
  绘制尺寸直接读 `shape`（动态实占矩形），连线端点按两端元素的真实形状贴合求交
  （`geometry.rayShapeExit`：中心沿边方向到轮廓的距离）—— 端点与渲染轮廓重合，
  不再使用等效圆近似裁剪。
...
"巨大号的单元"（单元形状 = 内部块实占的外接矩形并集 + 包裹边距）参与
外层布局，最后把内部块平移映射（自底向上）到单元最终位置。消费
`subgraphs` 与 `hiddenGroups` 声明（含嵌套）。
